const createTeamsChartData2 = async (teamNames, gp) => {
...
const createTeamsChart2 = async () => {
    const teams = await loadJsonData('data/teams.json');
    const gp = await loadJsonData('data/gp.json');
    if (!teams || !gp) return;

    const teamNames = ['FER', 'RED', 'MER']; // replace with your list of team names or IDs
    const chartData = await createTeamsChartData2(teamNames, gp);
    const ctx = document.getElementById('teamsChart2').getContext('2d');
