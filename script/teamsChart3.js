const createTeamsChartData3 = async (teamNames, gp) => {
...
const createTeamsChart3 = async () => {
    const teams = await loadJsonData('data/teams.json');
    const gp = await loadJsonData('data/gp.json');
    if (!teams || !gp) return;

    const teamNames = ['AST', 'ALP', 'HAA', 'RAC', 'WIL', 'SAU']; // replace with your list of team names or IDs
    const chartData = await createTeamsChartData3(teamNames, gp);
    const ctx = document.getElementById('teamsChart3').getContext('2d');
